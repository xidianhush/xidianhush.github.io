...
设函数 $f(x)$ 在 $x_0$ 的某去心邻域有定义。 若

$\boldsymbol{\lim_{x\to x_0} f(x)=0}$ 就称：**$f(x)$ 是 $x\to x_0$ 时的无穷小**。

👉 其他极限过程同理：$x\to\infty,\ x\to x_0^+,\ n\to\infty$，只要极限等于 0，
 
### 1.4.2 无穷大

$x\to x_0$ 时无穷大定义

设 $f(x)$ 在 $x_0$ 的某去心邻域有定义。

若：对任意大的正数$M>0$，总能找到$\delta>0$，当 $0<|x-x_0|<\delta$，

就有$|f(x)|>M$ 则称 $f(x)$ 是 $x\to x_0$ 时的**无穷大**，记作：
